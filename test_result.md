...
agent_communication:
  - agent: "main"
    message: "Исправлены 4 критические проблемы: 1) Установлен poppler-utils для корректной конвертации PDF в изображения, 2) Добавлены Open Graph мета-теги для исправления мобильной ошибки, 3) Реализована поддержка HTML форматирования: добавлено поле content_type в модель Contract, функция html_to_text_for_pdf() для конвертации HTML в текст при генерации PDF, 4) Обновлены CreateContractPage, SignContractPage и ContractDetailsPage для отображения HTML контента. Backend перезапущен успешно. Готов к тестированию."
  - agent: "testing"
    message: "✅ BACKEND ТЕСТИРОВАНИЕ ЗАВЕРШЕНО. Все 4 критические проблемы успешно исправлены и протестированы: 1) poppler-utils установлен и работает - PDF документы конвертируются в изображения без ошибок, 2) HTML форматирование полностью поддерживается - поле content_type сохраняется, HTML контент сохраняется как есть, 3) PDF генерация с HTML контентом работает корректно - HTML конвертируется в текст при генерации PDF, 4) PDF-to-Image конвертация через poppler работает без ошибок. Все backend API тесты пройдены успешно. Backend готов к продакшену."
...
agent_communication:
  ...
  - agent: "main"
    message: "🔧 ИСПРАВЛЕНИЕ ОШИБКИ КНОПКИ УТВЕРДИТЬ: Пользователь сообщил что при нажатии кнопки 'Утвердить' возникает ошибка. ПРИЧИНА: В эндпоинте POST /api/contracts/{contract_id}/approve-for-signing на строке 3221 вызывается несуществующая функция send_email_with_attachment(). ИСПРАВЛЕНИЕ: Заменен вызов send_email_with_attachment() на корректный вызов send_email() с правильными параметрами (to_email, subject, body, attachment, filename). Теперь утверждение договора должно работать и email с PDF должен отправляться клиенту."
  - agent: "main"

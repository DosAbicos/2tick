...
agent_communication:
  ...
  - task: "Исправление критического бага 405 Method Not Allowed при подписании контрактов"
    implemented: true
    working: true
    ...
    status_history:
      - working: "NA"
        agent: "main"
        comment: "КРИТИЧЕСКИЙ БАГ ИСПРАВЛЕН: Найдены и устранены ДВЕ проблемы, блокирующие подписание контрактов через все три метода верификации (SMS, Call, Telegram). ПРОБЛЕМА #1: SignContractPage.js:710 использовал axios.patch() для обновления placeholder_values перед верификацией, но backend эндпоинт PUT /api/contracts/{contract_id} (server.py:2032) принимает только PUT метод, что вызывало ошибку 405. ИСПРАВЛЕНИЕ #1: Изменен axios.patch → axios.put в SignContractPage.js:710. ПРОБЛЕМА #2: Эндпоинт POST /api/sign/{contract_id}/verify-otp (SMS верификация) на строке 3059 server.py возвращал только {message, signature_hash}, но НЕ возвращал поле verified:true. Frontend на строке 741 SignContractPage.js проверяет response.data.verified для перехода на Step 6, из-за чего после успешной SMS верификации переход не происходил. ИСПРАВЛЕНИЕ #2: Добавлено verified:true в ответ эндпоинта verify-otp (server.py:3059). Теперь все три метода верификации возвращают единообразный ответ с полем verified. Готов к тестированию через backend testing agent."

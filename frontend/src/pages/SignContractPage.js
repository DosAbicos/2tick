...
const SignContractPage = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setStep(2.5); // Go to final preview before verification
    } catch (error) {
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Card data-testid="sign-contract-card">
          ...
          <CardContent className="space-y-6">
            ...
            {/* Step 2.5: Final Contract Preview */}
            {step === 2.5 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                data-testid="step-final-preview"
              >
                <div className="mb-6 text-center">
                  <h3 className="text-2xl font-bold mb-2">Финальный просмотр договора</h3>
                  <p className="text-neutral-600">Проверьте все данные перед подписанием</p>
                </div>
                
                <div className="bg-gradient-to-br from-yellow-50 to-amber-50 border-2 border-yellow-400 rounded-lg p-4 mb-6">
                  <div className="flex items-start gap-3">
                    <div className="flex-shrink-0 w-10 h-10 bg-yellow-500 rounded-full flex items-center justify-center">
                      <span className="text-white text-lg">⚠️</span>
                    </div>
                    <div>
                      <p className="font-semibold text-yellow-900 mb-1">Внимательно проверьте договор</p>
                      <p className="text-sm text-yellow-800">
                        После подписания изменить данные будет невозможно. Убедитесь что все данные корректны.
                      </p>
                    </div>
                  </div>
                </div>

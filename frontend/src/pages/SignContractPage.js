          ...
          <CardContent className="space-y-6">
            ...
            {step === 1.5 && needsInfo && (
              ...
              >
                ...
                {/* Additional placeholder fields - editable */}
                {contract.content && (contract.content.includes('[Адрес квартиры]') || contract.content.includes('[Сумма аренды]') || contract.content.includes('[Дата заселения]')) && (
                  <div className="border-t pt-4 mt-4">
                    <h4 className="text-sm font-medium mb-3 text-neutral-700">Дополнительные данные договора</h4>
                    
                    {contract.content.includes('[Адрес квартиры]') && (
                      <div className="mb-3">
                        <Label htmlFor="property_address">Адрес квартиры</Label>
                        <Input
                          id="property_address"
                          value={signerInfo.property_address || ''}
                          onChange={(e) => setSignerInfo({...signerInfo, property_address: e.target.value})}
                          className="mt-1"
                    ...
                    {contract.content.includes('[Сумма аренды]') && (
                      <div className="mb-3">
                        <Label htmlFor="rent_amount">Сумма аренды</Label>
                        <Input
                          id="rent_amount"
                          value={signerInfo.rent_amount || ''}
                          onChange={(e) => setSignerInfo({...signerInfo, rent_amount: e.target.value})}
                          className="mt-1"
                          placeholder="150000 тенге"
                        />
                      </div>
                    )}
                    
                    {contract.content.includes('[Дата заселения]') && (
                      <div className="mb-3">
                        <Label htmlFor="move_in_date">Дата заселения</Label>
                        <Input
                          id="move_in_date"
                          type="date"
                          value={signerInfo.move_in_date || ''}
                          onChange={(e) => setSignerInfo({...signerInfo, move_in_date: e.target.value})}
                          className="mt-1"
                        />
                      </div>
                    )}
                    
                    {contract.content.includes('[Дата выселения]') && (
                      <div className="mb-3">
                        <Label htmlFor="move_out_date">Дата выселения</Label>
                        <Input
                          id="move_out_date"
                          type="date"
                          value={signerInfo.move_out_date || ''}
                          onChange={(e) => setSignerInfo({...signerInfo, move_out_date: e.target.value})}
                          className="mt-1"
                        />
                      </div>
                    )}
                    
                    {contract.content.includes('[Количество дней]') && (
                      <div className="mb-3">
                        <Label htmlFor="days_count">Количество дней</Label>
                        <Input
                          id="days_count"
                          type="number"
                          value={signerInfo.days_count || ''}
                          onChange={(e) => setSignerInfo({...signerInfo, days_count: e.target.value})}
                          className="mt-1"
                          placeholder="30"
                        />
                      </div>

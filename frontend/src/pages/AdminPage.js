      ...
      <Dialog open={contractViewOpen} onOpenChange={setContractViewOpen}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Детали договора и кнопка скачивания
            </DialogDescription>
          </DialogHeader>
          {selectedContract && (
            <div className="space-y-4">
              ...
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                <Label className="text-xs text-blue-600 font-semibold">ID договора</Label>
                <div className="flex items-center justify-between mt-1">
                  <code className="text-sm font-mono text-blue-900">{selectedContract.id}</code>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium">Код договора</Label>
                  <p className="text-sm text-neutral-600 font-mono bg-neutral-50 px-2 py-1 rounded">
                    {selectedContract.contract_code || 'Не указан'}
                  </p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Статус</Label>
                ...
                <div className="col-span-2">
                  <Label className="text-sm font-medium">Название</Label>
                  <p className="text-sm text-neutral-600">{selectedContract.title}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Дата создания</Label>
                  <p className="text-sm text-neutral-600">
                    {new Date(selectedContract.created_at).toLocaleDateString('ru-RU', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                      hour: '2-digit',
                ...
                <div>
                  <Label className="text-sm font-medium">Наймодатель</Label>
                  <p className="text-sm text-neutral-600">{selectedContract.landlord_name || 'Не указан'}</p>
                </div>
                {selectedContract.signer_name && (
                  <div>
                    <Label className="text-sm font-medium">Наниматель</Label>
                    <p className="text-sm text-neutral-600">{selectedContract.signer_name}</p>
                  </div>
                )}
                {selectedContract.signer_phone && (
                  <div>
                    <Label className="text-sm font-medium">Телефон нанимателя</Label>
                    <p className="text-sm text-neutral-600">{selectedContract.signer_phone}</p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t">
                <Label className="text-sm font-medium mb-2 block">Содержание договора (превью)</Label>
                <div className="bg-neutral-50 p-4 rounded-lg border max-h-64 overflow-y-auto">
                  <div 
                    className="text-sm text-neutral-700 whitespace-pre-wrap"
                    dangerouslySetInnerHTML={{ 
                      __html: selectedContract.content?.substring(0, 1000) + (selectedContract.content?.length > 1000 ? '...' : '') 
                    }}
                  />
                </div>
              </div>
              ...
              <div className="p-4 bg-green-50 border-2 border-green-300 rounded-lg flex items-center justify-between">
                <div>
                  <p className="font-semibold text-green-900">Скачать полный договор в PDF</p>
                  <p className="text-sm text-green-700">Нажмите на кнопку справа для скачивания</p>
                </div>
                <Button
                  onClick={() => downloadContract(selectedContract.id)}
                  size="lg"
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setContractViewOpen(false)}>
              Закрыть
            </Button>
          </DialogFooter>
        </DialogContent>

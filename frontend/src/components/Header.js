    ...
    <header className="border-b border-gray-200/50 bg-white/90 backdrop-blur-md sticky top-0 z-50 shadow-sm" data-version="v3">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 h-14 md:h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 md:gap-3">
          {/* Языковой селектор - скрыт для страницы подписания */}
                          ...
                          if (isAdminPage) {
                            ...
                            return (
                              <Link to="/admin" onClick={() => setMobileMenuOpen(false)}>
                                <button className="w-full py-4 px-5 text-base font-semibold text-red-700 bg-white rounded-2xl hover:shadow-xl transition-all flex items-center gap-4 shadow-md" style={{boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06), inset 0 -2px 4px rgba(0, 0, 0, 0.05)'}}>

...
def generate_contract_pdf(contract: dict, signature: dict = None, landlord_signature_hash: str = None, landlord: dict = None, template: dict = None) -> bytes:
    ...
    # Signatures section - Professional design with logo
    if signature or landlord_signature_hash:
        ...
        box_x = 35
        box_y = y_position - 500  # Height of signature section
        box_width = width - 70
        box_height = 520
        ...
        if os.path.exists(logo_path):
            try:
                ...
                logo_y = y_position - 55
                
                img_reader = ImageReader(logo_path)
                p.drawImage(img_reader, logo_x, logo_y, width=logo_width, height=logo_height, mask='auto')
            except Exception as e:
        ...
        if landlord_signature_hash:
            ...
            party_a_role = get_role_by_language(contract, 'party_a_role', selected_language)
            ...
            p.drawString(landlord_x, y_landlord, landlord_signature_hash[:35])
            y_landlord -= 10
            if len(landlord_signature_hash) > 35:
                p.drawString(landlord_x, y_landlord, landlord_signature_hash[35:])
            ...
            # Contract language
            p.setFillColor(HexColor('#64748b'))
            p.drawString(landlord_x, y_landlord, "Язык договора:")
            y_landlord -= 13
            ...
            p.drawString(landlord_x, y_landlord, lang_display)
        
        # RIGHT COLUMN - Tenant (Party B)
        if signature and signature.get('verified'):
            ...
            # Use dynamic role from contract in appropriate language
            party_b_role = get_role_by_language(contract, 'party_b_role', selected_language)
            
            # Draw role text (no background)
            p.setFillColor(HexColor('#000000'))  # Black
            p.drawString(tenant_x, y_tenant, f"✓ {party_b_role}")
            ...
            # If no template but contract has placeholder_values, try to infer tenant fields
            if not template and contract.get('placeholder_values'):
                print(f"🔥 PDF DEBUG - No template, using placeholder_values directly for tenant")
                # Common tenant placeholder keys
                tenant_keys = [
                    ('ФИО_НАНИМАТЕЛЯ', 'ФИО Нанимателя'),
                    ('ИИН_КЛИЕНТА', 'ИИН клиента'),
                    ('EMAIL_КЛИЕНТА', 'Почта клиента'),
                    ('НОМЕР_КЛИЕНТА', 'Номер клиента'),
                    ('КОЛИЧЕСТВО_ЧЕЛОВЕК', 'Количество человек'),
                    ('ТОЛЬКО_В_ИНФОРМАЦИИ_ПОДПИСАНИИ_И_В_ДОГОВОРЕ', 'Только в информации о подписании'),
                ]
                
                for key, label in tenant_keys:
                    if key in contract['placeholder_values']:
                        value = contract['placeholder_values'][key]
                        if value:
        ...
        # Add minimal footer with security information
        footer_y = box_y + 35
        ...
        p.setFillColor(HexColor('#9ca3af'))  # Gray
    ...
    # Page number and website (no decorative line)
    p.drawString(40, 20, f"Страница {page_num}")
    p.drawRightString(width - 40, 20, "2tick.kz")
